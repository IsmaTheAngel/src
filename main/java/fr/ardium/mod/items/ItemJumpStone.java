...
public class ItemJumpStone extends Item
{
	
public ItemJumpStone()
	{
		 setMaxDamage(32);
		 setMaxStackSize(1);
	}
	 ...
	 public void onUpdate(ItemStack item, World world, Entity player, int slotIndex, boolean inHand)
	  ...
	  public ItemStack onItemRightClick(ItemStack item, World world, EntityPlayer player)
	  {
	    ...
	    if (item.stackTagCompound.getInteger("timer") == 0)
	    {
	      ...
	      player.addPotionEffect(new PotionEffect(Potion.jump.id, 200, 9));
	      item.stackTagCompound.setInteger("timer", 1);
	    }
	    else if (world.isRemote)
	    {
	      player.addChatComponentMessage(new ChatComponentTranslation(EnumChatFormatting.DARK_GREEN + "Tu dois attendre que la pierre se recharge !", new Object[0]));
	    }
	    return item;
	  }
