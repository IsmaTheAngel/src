...
import fr.ardium.mod.until.AOEHarvestTool;
...
public class ToolAtomicDissasembler 
extends AOEHarvestTool
{
	  public ToolAtomicDissasembler(ToolMaterial material)
	  {
	    super(1, 1, 0);
	  }
	  ...
	  static Material[] materials = { Material.rock, Material.iron, Material.ice, Material.glass, Material.piston, Material.anvil,Material.wood,Material.grass,Material.sand,Material.ground };
	      ...
	      if ((!check.contains("sand")) && (!check.contains("stonebrick")) && (!check.contains("hellrock")) && (!check.contains("clay")) && (!check.contains("stonebricksmooth")) && (!check.contains("cactus")) && (!check.contains("stonebrick"))) {
	        for (int x = event.drops.size() - 1; x >= 0; x--) {
	          if (FurnaceRecipes.smelting().getSmeltingResult((ItemStack)event.drops.get(x)) != null)
	          {
	            ...
	            if ((breaker == RegistryTools.atomic_dissasembler)  && (breaker != null) && (this.hasSilkTouch != true))
	            {
