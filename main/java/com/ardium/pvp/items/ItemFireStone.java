...
class ItemFireStone extends Item {
    ...
    public ItemStack onItemRightClick(ItemStack item, World world, EntityPlayer player) {
        ...
        if ( item.stackTagCompound.getInteger ("timer") == 0 ) {
            ...
            player.addPotionEffect (new PotionEffect (Potion.fireResistance.id, 200, 20));
            item.stackTagCompound.setInteger ("timer", 1);
        } else if ( world.isRemote ) {
            player.addChatComponentMessage (new ChatComponentTranslation (EnumChatFormatting.GOLD + "Tu dois attendre que la pierre se recharge !"));
        }
        return item;
    }
    ...
    public boolean hasEffect(ItemStack item, int metadata) {
        return true;
    }
