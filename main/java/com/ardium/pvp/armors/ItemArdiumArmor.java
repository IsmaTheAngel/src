package com.ardium.pvp.armors;

import com.ardium.pvp.Ardium;
import com.ardium.pvp.StackUntil;
import com.ardium.pvp.items.RegistryItems;
import net.minecraft.entity.Entity;
import net.minecraft.item.ItemArmor;
import net.minecraft.item.ItemStack;

class ItemArdiumArmor
        extends ItemArmor {

    public ItemArdiumArmor(ArmorMaterial material, int p_i45325_3_) {
        super (material, 0, p_i45325_3_);
    }

    public String getArmorTexture(ItemStack stack, Entity entity, int slot, String type) {
        if ( slot == 2 ) {
            return Ardium.MOD_ID + ":textures/models/armor/ardium_layer_2.png";
        }
        return Ardium.MOD_ID + ":textures/models/armor/ardium_layer_1.png";
    }

    public boolean getIsRepairable(ItemStack stack1, ItemStack stack2) {
        return StackUntil.equalsWildcard (getRepairStack (), stack2) || super.getIsRepairable (stack1, stack2);
    }

    private ItemStack getRepairStack() {
        if ( (getArmorMaterial () == RegistryArmors.Ardium) ) {
            return new ItemStack (RegistryItems.ardium_ingot, 1, 0);
        }

        return new ItemStack (getArmorMaterial ().func_151685_b ());

    }

}

